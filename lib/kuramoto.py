...
edge_file = f'../../kur/edges_c{c}_N{G.number_of_nodes()}.csv'
pd.DataFrame(edge_list).to_csv(edge_file)

G = nx.Graph()
G.add_edges_from(pd.read_csv(edge_file,index_col=0).to_numpy().T)
...
x = np.array(x).T
ax = pd.DataFrame(x.T).diff().iloc[steps//4::100,:50].plot(legend=False,logx=False)
ax.set_ylabel(r'$\theta_i$', rotation=90, fontsize=30); ax.set_xlabel(r'$t$', rotation=0, fontsize=30)
plt.savefig(f'../../kur/kur_K{K}_N{A.shape[0]}_c{c}_time{int(tic)}.pdf')
...
plt.savefig(f'../../kur/order_K{K}_N{A.shape[0]}_c{c}_time{int(tic)}.pdf')
...
idx = [1,5,2]#list(np.random.choice(len(s), 3, False))
...
plt.savefig(f'../../kur/spectrum_K{K}_N{A.shape[0]}_c{c}_time{int(tic)}.pdf')

pd.DataFrame(A).to_csv(f'../../kur/adj_K{K}_N{A.shape[0]}_c{c}_time{int(tic)}.csv')
pd.DataFrame(x.T,columns=omega_i).to_csv(f'../../kur/kur_K{K}_e{eps}_N{A.shape[0]}_c{c}_time{int(tic)}.csv')
